...
const connection = DB.connection
  ...
  router.get('/', function(req, res) {
    ...
    if(data.articleType == '') { // 分类为空即查询个人所有评论
      countSql = 'SELECT COUNT(*) FROM comment_table WHERE username=?';
      commentSql = 'SELECT comment_table.*,article_table.title,category_table.category_name FROM category_table,article_table,comment_table '+
                   'WHERE comment_table.username=? AND comment_table.article_id = article_table.ID AND article_table.category_id=category_table.ID ORDER BY ID DESC ' +
      ...
      commentSql = 'SELECT comment_table.*,article_table.title,category_table.category_name FROM category_table,article_table,comment_table '+
                   'WHERE article_table.username=? AND comment_table.article_id='+data.articleType+' AND comment_table.article_id = article_table.ID AND article_table.category_id=category_table.ID ORDER BY ID DESC ' +
                   'limit ' + data.limit + ' offset ' + data.limit * (data.current_page - 1)
    }
    
    var countPromise = connection(db, countSql, [data.userInfo.username]);
    var commentPromise = connection(db, commentSql, [data.userInfo.username]);
    ...
    Promise.all([countPromise, commentPromise])
      .then(function(result) {
        ...
        res.render('my/comment_index.ejs', data)
      }).catch(function(err) {
        console.log(err)
